      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-10 gap-8">
        {games.slice(0,10).map(game => (
          <div key={game.id} className="bg-slate-800 rounded-lg border-2 border-gray-800">
            <img className="rounded-t-lg border-b-2 border-gray-700 w-full aspect-3/4 object-cover bg-slate-900" src={game.cover_url} />
            <p className="font-bold text-indigo-400 ml-2 mt-1">{new Date(game.first_release_date*1000).toLocaleDateString()}</p>
            <p className="font-semibold text-white ml-2 mb-2">{game.name}</p>
...
function App() {
  ...
  return (
    <main className="flex-1 p-8 bg-slate-950"> 
      ...
      <GameRow heading="Upcoming" endpoint="/upcoming-releases" />
    </main>
